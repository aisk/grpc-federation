...
func (e *encoder) toValue(value *resolver.Value) *plugin.Value {
	...
	switch {
	case value.CEL != nil:
		ret.Value = &plugin.Value_Cel{
			Cel: e.toCELValue(value.CEL),
		}
	case value.Const != nil:
		ret.Value = &plugin.Value_Const{
			Const: e.toConstValue(value.Const),
		}
	}
	return ret
}

func (e *encoder) toConstValue(value *resolver.ConstValue) *plugin.ConstValue {
	if value == nil {
		return nil
	}
	ret := &plugin.ConstValue{
		Type: e.toType(value.Type),
	}
	v := value.Value
	switch value.Type.Kind {
	case types.Double:
		if value.Type.Repeated {
			ret.Doubles = v.([]float64)
		} else {
			vv := v.(float64)
			ret.Double = &vv
		}
	...
	case types.String:
		if value.Type.Repeated {
			...
			case []string:
				ret.Strings = vv
			case []resolver.EnvKey:
				strs := make([]string, 0, len(vv))
				for _, vvv := range vv {
					strs = append(strs, string(vvv))
				}
				ret.Envs = strs
			}
		} else {
	...
	case types.Bytes:
		if value.Type.Repeated {
			ret.ByteStrings = v.([][]byte)
		} else {
			ret.ByteString = v.([]byte)
		}
	case types.Uint32:
		if value.Type.Repeated {
			ret.Uint32S = v.([]uint32)
		} else {
			vv := v.(uint32)
			ret.Uint32 = &vv
		}
	case types.Enum:
		if value.Type.Repeated {
			var enums []string
			for _, vv := range v.([]*resolver.EnumValue) {
				enums = append(enums, e.toEnumValueID(vv))
			}
			ret.Enums = enums
		} else {
			value := e.toEnumValueID(v.(*resolver.EnumValue))
			ret.Enum = &value
		}
	case types.Sfixed32:
		if value.Type.Repeated {
			ret.Sfixed32S = v.([]int32)
		} else {
			vv := v.(int32)
			ret.Sfixed32 = &vv
		}
	...
	case types.Message:
		if value.Type.Repeated {
			var msgs []*plugin.MessageValue
			for _, msg := range v.([]map[string]*resolver.Value) {
				msgs = append(msgs, e.toMessageValue(msg))
			}
			ret.Messages = msgs
		} else {
			ret.Message = e.toMessageValue(v.(map[string]*resolver.Value))
		}
	}
	return ret
}

func (e *encoder) toMessageValue(msg map[string]*resolver.Value) *plugin.MessageValue {
	if msg == nil {
		return nil
	}
	var fields []*plugin.MessageFieldValue
	for k := range msg {
		fields = append(fields, &plugin.MessageFieldValue{
			Field: k,
		})
	}
	return &plugin.MessageValue{
		Fields: fields,
	}
}
