		...
		Message: func(ctx context.Context, value *localValueType) (any, error) {
			...
			if err := grpcfed.SetCELValue(ctx, &grpcfed.SetCELValueParam[*CreatePost]{
				...
				Setter: func(v *CreatePost) error {
					Post, err := s.cast_Org_Federation_CreatePost__to__Org_Post_CreatePost(v)
					if err != nil {
						return err
					}
					args.Post = Post
	...
	*/
	...
	if err := grpcfed.SetCELValue(ctx, &grpcfed.SetCELValueParam[*post.Post]{
		...
		Setter: func(v *post.Post) error {
			Post, err := s.cast_Org_Post_Post__to__Org_Federation_Post(v)
			if err != nil {
				return err
			}
			ret.Post = Post
...
func (s *FederationService) cast_Org_Federation_CreatePost__to__Org_Post_CreatePost(from *CreatePost) (*post.CreatePost, error) {
	...
	Type, err := s.cast_Org_Federation_PostType__to__Org_Post_PostType(from.GetType())
	if err != nil {
		return nil, err
	}
	PostType, err := s.cast_Org_Federation_PostType__to__int32(from.GetPostType())
	if err != nil {
		return nil, err
	}
