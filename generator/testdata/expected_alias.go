...
func (s *FederationService) resolve_Org_Federation_Post(ctx context.Context, req *Org_Federation_PostArgument) (*Post, error) {
	...
	*/
	...
	{
		Data, err := s.cast_Org_Post_PostData__to__Org_Federation_PostData(value.vars.post.GetData()) // { name: "post", autobind: true }
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
...
func (s *FederationService) cast_Org_Post_PostContent__to__Org_Federation_PostContent(from *post.PostContent) (*PostContent, error) {
	...
	Category, err := s.cast_Org_Post_PostContent_Category__to__Org_Federation_PostContent_Category(from.GetCategory())
	if err != nil {
		return nil, err
	}
	Head := from.GetHead()
...
func (s *FederationService) cast_Org_Post_PostData__to__Org_Federation_PostData(from *post.PostData) (*PostData, error) {
	...
	Type, err := s.cast_Org_Post_PostDataType__to__Org_Federation_PostType(from.GetType())
	if err != nil {
		return nil, err
	}
	Title := from.GetTitle()
	Content, err := s.cast_Org_Post_PostContent__to__Org_Federation_PostContent(from.GetContent())
	if err != nil {
		return nil, err
	}
