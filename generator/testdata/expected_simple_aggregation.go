	...
	*/
	...
	if err := grpcfed.SetCELValue(ctx, &grpcfed.SetCELValueParam[Item_ItemType]{
		...
		Setter: func(v Item_ItemType) error {
			EnumValue, err := s.cast_Org_Federation_Item_ItemType__to__int32(v)
			if err != nil {
				return err
			}
...
func (s *FederationService) resolve_Org_Federation_User(ctx context.Context, req *Org_Federation_UserArgument) (*User, error) {
	...
	*/
	...
	{
		Type, err := s.cast_Org_User_UserType__to__Org_Federation_UserType(value.vars.user.GetType()) // { name: "user", autobind: true }
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	{
		MainItem, err := s.cast_Org_User_Item__to__Org_Federation_Item(value.vars.user.GetMainItem()) // { name: "user", autobind: true }
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
		ret.MainItem = MainItem
	}
	{
		Items, err := s.cast_repeated_Org_User_Item__to__repeated_Org_Federation_Item(value.vars.user.GetItems()) // { name: "user", autobind: true }
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	case value.vars.user.GetAttrA() != nil:
		...
		Attr, err := s.cast_Org_User_User_AttrA___to__Org_Federation_User_AttrA_(value.vars.user.GetAttrA())
		if err != nil {
			return nil, err
		}
		ret.Attr = Attr
	case value.vars.user.GetB() != nil:
		...
		Attr, err := s.cast_Org_User_User_B__to__Org_Federation_User_B(value.vars.user.GetB())
		if err != nil {
			return nil, err
		}
		ret.Attr = Attr
...
func (s *FederationService) cast_Org_User_Item__to__Org_Federation_Item(from *user.Item) (*Item, error) {
	...
	Name := from.GetName()
	Type, err := s.cast_Org_User_Item_ItemType__to__Org_Federation_Item_ItemType(from.GetType())
	if err != nil {
		return nil, err
	}
	Value := from.GetValue()
...
func (s *FederationService) cast_Org_User_User_AttrA___to__Org_Federation_User_AttrA_(from *user.User_AttrA) (*User_AttrA_, error) {
	...
	AttrA, err := s.cast_Org_User_User_AttrA__to__Org_Federation_User_AttrA(from)
	if err != nil {
		return nil, err
	}
	return &User_AttrA_{AttrA: AttrA}, nil
}
...
func (s *FederationService) cast_Org_User_User_B__to__Org_Federation_User_B(from *user.User_AttrB) (*User_B, error) {
	...
	B, err := s.cast_Org_User_User_AttrB__to__Org_Federation_User_AttrB(from)
	if err != nil {
		return nil, err
	}
	return &User_B{B: B}, nil
}
